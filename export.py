...
def write_awards(payload: dict, site_dir: str | Path) -> Path:
    """Emit awards.js, stamped the same way data.js is."""
    site_dir = Path(site_dir)
    (site_dir / "data").mkdir(parents=True, exist_ok=True)
    out = site_dir / "data" / "awards.js"
    out.write_text(
        "window.XDAWG_AWARDS = " + json.dumps(payload, separators=(",", ":")) + ";\n",
        encoding="utf-8",
    )
    _stamp_cache_buster(site_dir, payload.get("generated", ""),
                        page="awards.html", data="data/awards.js")
    return out
