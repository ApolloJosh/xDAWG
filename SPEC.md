...
**Resolved.** `score_pillar(..., self_only=True)` now drops those components
from the wDAWG pass and renormalizes the survivors, so wDAWG+ means strictly
what it claims — how much a player beat his own baseline when it mattered —
and DAWG+ keeps everything, which is where "how good is he" belongs. The two
stats are now different questions rather than a quarter the same one; on
synthetic data their correlation fell from +0.317 to +0.126.

### Not being a different player is not a flaw

A speedster posts speed numbers and a slugger posts power numbers, and neither
is a statement about whether the man competes. Two components were charging
players for their body type:

**Baserunning.** `extra_bases_taken` compared every runner to the league's
average advancement for that base and hit type, which made it substantially a
footrace — a slow slugger scored badly for not going first-to-third. The
baseline is now per (base, hit type, **speed group**), five groups cut from
sprint speed, so the question is whether he takes the extra base more often
than other runners of his speed. That is a question about reads and
aggression. `hustle_ratio` always did this correctly — home-to-first time as
a fraction of the player's OWN top speed — so this brings baserunning into
line with the pillar's own precedent rather than inventing a rule.

**Defense.** `oaa_situational`'s shrinkage constant was 40 fielding chances,
against a full-time defender's 300–450. That trusted a full-timer at 90% and a
man who spells someone at first twice a week at 60% — nominally "scaled by how
much he actually fielded," in practice a rounding error, and it carried a real
negative defensive z into a bat-first slugger's score. At k=150 a full-timer
keeps ~70% of his defensive signal and an occasional fielder under 30%.

Neither change touches the leverage deltas, which were never the problem: the
null simulation above shows those are unbiased at every talent level.
