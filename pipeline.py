...
def run(
    ...
    p = add_leverage(p)
    return run_frames(p, season, min_pa=min_pa, min_bf=min_bf)
...
def run_frames(
    ...
    """Score an ALREADY-LOADED pitch frame that already carries leverage.

    Split out of `run` so the awards can score an arbitrary slice -- one day,
    one week, one month -- without re-fetching anything and, more importantly,
    without re-deriving the leverage index from the slice. Leverage estimated
    from four days of games would be worthless, so the season's `li` rides
    along on the rows and this never touches it.

    `names` is optional because the reverse lookup is a network call: a
    window pass only needs the numbers, and the ids are already known.
    """
    min_pa = QUALIFY["hitter_min_pa"] if min_pa is None else min_pa
    min_bf = QUALIFY["pitcher_min_bf"] if min_bf is None else min_bf
    ...
    say(f"[xdawg] scoring {len(hit)} hitters, {len(pit)} pitchers")
